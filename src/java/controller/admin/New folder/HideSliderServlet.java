...
@WebServlet(name = "HideSliderServlet", urlPatterns = {"/hideSlider"})
public class HideSliderServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (sliderIdStr != null && !sliderIdStr.trim().isEmpty()) {
            try {
                ...
                boolean success = sliderDAO.updateSliderStatus(sliderId, "inactive");
                
                if (success) {
                    response.sendRedirect(request.getContextPath() + "/sliderlist?message=Slider hidden successfully!");
                } else {
                    response.sendRedirect(request.getContextPath() + "/sliderlist?error=Failed to hide slider. Please try again.");
                }
