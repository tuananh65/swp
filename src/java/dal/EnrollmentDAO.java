...
import model.Course;
// Import Log4j nếu cần, nhưng tạm thời sẽ comment để troubleshoot
...
public class EnrollmentDAO extends DBContext {
    ...
    // Tạm thời comment khởi tạo Logger để loại trừ lỗi Log4j
    ...
    public boolean insertEnrollment(Enrollment e) {
        ...
        try {
            ...
            // Tạm thời thay thế logger bằng System.out.println
            // logger.debug("Executing insertEnrollment for userID: {}, courseID: {}", e.getUserId(), e.getCourseId());
            System.out.println("Executing insertEnrollment for userID: " + e.getUserId() + ", courseID: " + e.getCourseId());

            return ps.executeUpdate() > 0;
        } catch (SQLException ex) {
            // ex.printStackTrace(); // Có thể thay bằng logger.error, nhưng để troubleshoot thì printStackTrace cũng được
            // logger.error("Error inserting enrollment: {}", ex.getMessage(), ex);
            ex.printStackTrace();
        } finally {
            try {
                if (ps != null) ps.close();
