...
public class EnrollmentDAO extends DBContext {
    ...
    public int getTotalEnrollmentsWithFilterByUserId(
            int userId, String subject, String status,
...
// lấy danh sách có lọc 
    ...
    public boolean updateEnrollment(Enrollment e) {
        String sql = "UPDATE Enrollment SET Status = ?, PackageID = ?, ValidFrom = ?, ValidTo = ?, Note = ? WHERE EnrollmentID = ?";
        try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public static void main(String[] args) {
        ...
        //  Điều kiện lọc 
        String subject = null; //  "Java"
        String status = null;  //  "Approved"
        Date fromDate = null;  //  Date.valueOf("2024-01-01");
        Date toDate = null;    //  Date.valueOf("2024-12-31");
        BigDecimal minPrice = new BigDecimal("0"); // Giới hạn dưới của giá
        BigDecimal maxPrice = new BigDecimal("100000000"); // Giới hạn trên

        String sortColumn = "EnrollmentDate";
        String sortOrder = "desc";
        int offset = 0;
        int limit = 10;

        // Gọi hàm lọc DTO
        List<EnrollmentDTO> list = dao.getListEnrollmentsWithFilterByUserId(
                userId,
                subject, status,
                fromDate, toDate,
                minPrice, maxPrice,
        ...
        if (list.isEmpty()) {
            ...
            for (EnrollmentDTO dto : list) {
                System.out.println("-----------");
                System.out.println("ID: " + dto.getEnrollmentId());
                System.out.println("User: " + dto.getUserEmail());
                System.out.println("Course: " + dto.getCourseName());
                System.out.println("Package: " + dto.getPackageName());
                System.out.println("Total: " + dto.getTotalPrice() + " ₫");
                System.out.println("Status: " + dto.getStatus());
                System.out.println("From: " + dto.getValidFrom());
                System.out.println("To: " + dto.getValidTo());
                System.out.println("Updated By: " + dto.getUpdatedByName());
            }
        }
        Enrollment e = dao.getEnrollmentById(1);
