...
<script>
    ...
    searchInput.addEventListener('input', function () {
        ...
        <main class="main-content">
            ...
            <c:if test="${not empty course}">
                <div class="course-layout">
                    ...
                    <div class="course-info">
                        ...
                        <form action="${pageContext.request.contextPath}/myRegistrations" method="post">
                            <input type="hidden" name="action" value="register">
                            <input type="hidden" name="courseId" value="${course.courseID}">
                            <label for="packageId">Choose Package:</label>
                            <select name="packageId" id="packageId" required>
                                <c:forEach var="pkg" items="${packageList}">
                                    <option value="${pkg.packageId}">${pkg.name} - ${pkg.durationInDays} days</option>
                                </c:forEach>
                            </select>
            ...
            <!-- Inside the main-content section, add this conditional block -->
<c:if test="${not empty param.action && param.action == 'edit' && not empty enrollment}">
    <div class="course-layout">
        <h2>Edit Registration</h2>
        <form action="${pageContext.request.contextPath}/myRegistrations?action=update" method="post">
            <input type="hidden" name="enrollmentId" value="${enrollment.enrollmentId}">
            <div class="desc-block">
                <label>Course:</label>
                <input type="text" name="courseName" value="${enrollment.courseName}" readonly>
            </div>
            <div class="desc-block">
                <label>Package:</label>
                <select name="packageId">
                    <option value="1" ${enrollment.packageId == 1 ? 'selected' : ''}>Standard</option>
                    <option value="2" ${enrollment.packageId == 2 ? 'selected' : ''}>Premium</option>
                </select>
            </div>
            <div class="desc-block">
                <label>Valid From:</label>
                <input type="date" name="validFrom" value="<fmt:formatDate value='${enrollment.validFrom}' pattern='yyyy-MM-dd' />">
            </div>
            <div class="desc-block">
                <label>Valid To:</label>
                <input type="date" name="validTo" value="<fmt:formatDate value='${enrollment.validTo}' pattern='yyyy-MM-dd' />">
            </div>
            <div class="desc-block">
                <label>Note:</label>
            ...
            <a href="${pageContext.request.contextPath}/myRegistrations?courseId=${course.courseID}" class="cancel-btn">Cancel</a>
        </form>
    </div>
</c:if>
<!-- Existing course detail content remains below -->
        </main>
        <!-- Footer -->
